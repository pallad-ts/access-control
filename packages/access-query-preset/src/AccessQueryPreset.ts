...
export class AccessQueryPreset<T extends [string, unknown]> {
	...
	private isValidSubject(subject: unknown): subject is AccessQueryPreset.Subject<T[1]> {
		// eslint-disable-next-line no-null/no-null
		return typeof subject === 'object' && subject !== null && 'subjectType' in subject && (subject as any).subjectType === this.subjectType;
	}

	wrapSubjectWithSubjectType<T>(wrappedSubject: T): AccessQueryPreset.Subject<T> {
	...
	createPolicy<TPrincipal>(
		policyFunction: Policy<AccessQueryPreset.PolicyAccessQueryInput<TPrincipal, T>>
	): Policy<AccessQueryPreset.PolicyAccessQueryOutput<TPrincipal, T>> {
		return (query: AccessQueryPreset.PolicyAccessQueryOutput<TPrincipal, T>) => {
